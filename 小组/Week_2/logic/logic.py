# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 20:25:06 2024
...
def getdata():
    ...
    dataSet = []
    labels = []
    f = open('watermelon.csv')#数据集包括甜度，密度，以及西瓜好坏（布尔值）
    flag = 1  # 设置flag来跳过第一行
...
for i in range(p):
    if int(all_labels[i])==1:
        ...
        y2.append(float(data_arr[i,2]))#传输坐标，x为西瓜参数    
fig=plt.figure()
ax=fig.add_subplot(111)
ax.scatter(x1,y1,s=30,c='red',marker='s')
ax.scatter(x2,y2,s=30,c='green')#绿色为甜度，红色为密度
x=arange(-1.5,1.5,0.1)
# print(x)
y=(-weights[0][0][0]-weights[0][1][0]*x)/weights[0][2][0]#画出回归直线
ax.plot(x,y)
plt.show()
print("训练结果为：", result_weight)
print("准确率:", rate)